...
In-paper anchor: `paper/KAPLANSKY_IDEMPOTENT_CONJECTURE_PREPRINT.md` (`KID_G1`).

## Goal
Make the projected response floor explicit as the protected-core gate for `proving absence of nontrivial idempotents in admissible group algebras through an admissible algebraic-rigidity closure architecture`.
...
- projected core: the response sector controlled by `kappa_algebra`.
...
`KID_G1` closes when `kappa_algebra` satisfies the response-floor requirement: projected algebra response has a strict positive floor.
This is the protected-core contribution to the strict margin `M_KID`.
...
Payload: verify that all quantities used by `kappa_algebra` are defined on the projected sector named in the main preprint.
...
Payload: check the artifact key `kappa_algebra` and the corresponding extraction input/provenance record.

### Theorem EG1.3 (core gate closure)
If Lemmas EG1.1-EG1.2 hold and the runtime artifact accepts `kappa_algebra`, then `KID_G1` supplies the projected/protected-core input to `M_KID`.

## Current Instantiation

- gate: `KID_G1`
- artifact key: `kappa_algebra`
